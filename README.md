...
- **`roll_call_on_read_fail`** (optional, default `true`): when a group read fails, read one byte from every device individually (3 ms timeout each) and log which devices answer and which stay silent, on the first failure and every 100 ms while reads keep failing. If no device answers, the port is reopened once and asked again, which separates a dead port session (answers after reopen) from a bus fault (still silent). Also logs when reads recover.
